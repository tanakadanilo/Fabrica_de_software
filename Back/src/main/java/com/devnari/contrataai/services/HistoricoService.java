...
public class HistoricoService {
	...
	public HistoricoServico criarHistorico(PropostaContratacao propostaContratacao) {
		HistoricoServico historicoServico = new HistoricoServico(propostaContratacao.getPrestador(),
				...
				propostaContratacao.getDataContratacao(), null, StatusServico.EM_ANALISE);
	...
	public Double calcularMediaNotas(Prestador prestador) {
		...
		prestador.getServicosPrestados().forEach(x -> {
			media[0] += historicoServicoDao.calcularMediaNotasServico(x) == null ? 0
					: historicoServicoDao.calcularMediaNotasServico(x);
		});
		if (media[0] != 0) {
	...
	public HistoricoServico alterarStatusServico(Long idHistorico, StatusServico statusNovo) {
		HistoricoServico historicoServico = historicoServicoDao.findById(Integer.parseInt(idHistorico.toString()))
				.get();
		historicoServico.setStatus(statusNovo);
		return historicoServicoDao.save(historicoServico);
